        ...
        <section className="h-screen">
          <Modal show={isclicked} clicked={() => setClicked(false)}>
            <div className="my-5 text-2xl font-bold">
              <div>🍀 </div>
              <h1 className="mt-2 text-center">계정 만들기</h1>
            </div>
            <div>
              <span>로그인을 함으로써 연고링 이용약관에 동의합니다.<br /> 연고링은 학교 인증 기반 웹 어플리케이션으로서 인증하기 위해서는 학교 메일로 인증해야합니다.</span>
            </div>
            <div>
              <button onClick={() => console.log('clicked')} className="font-sans w-full border-2 rounded-3xl px-5 py-3 mt-10">학교 이메일로 인증하기</button>
            </div>
          </Modal>
          <div 
            style={{
