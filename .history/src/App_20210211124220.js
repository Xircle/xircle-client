...
import Modal from './Components/UI/backdrop';
    ...
    <>
      <Layout>
        <section className="h-screen">
          <Backdrop show={isclicked} clicked={() => setClicked(false)}/>
          <div 
            style={{
          ...
          >
            <div className="flex flex-col items-center w-full h-full" style={{backgroundColor: 'rgba(0, 0, 0, 0.4)'}}>
              ...
              <button onClick={() => setClicked(true)} className="font-sans text-white w-48 h-16 mt-10 rounded-3xl bg-transparent-600 border-2 hover:bg-gray-600 focus:outline-none">계정 만들기</button>
            </div>
          </div>
        </section>
      </Layout>
