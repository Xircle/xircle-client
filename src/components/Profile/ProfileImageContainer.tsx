...
import { getFriendThunk } from '../../store/modules/friend';
...
function ProfileImageContainer({ ...data }: Props) {
  const {
    profileImgSrc,
    adj,
    job,
  ...
  return (
    <>
      <ProfileImageContainerBlock>
        <ProfileImageBlock>
          <ProfileImage src={profileImgSrc} alt="profile image" />
          <Link to="/friend-profile">
            <ArrowForward
              onClick={() => dispatch(getFriendThunk('0'))}
              style={{
                backgroundColor: '#F9F9F9',
                padding: -10,
                bowShadow:
                  '-10px -10px 30px #FFFFFF, 10px 10px 20px rgba(174, 174, 192, 0.3)',
                borderRadius: '98px',
                position: 'absolute',
                top: '50%',
