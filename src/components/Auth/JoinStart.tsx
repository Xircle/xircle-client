...
export default function JoinStart ({ onNext, ...props}: Props) {
    ...
    const passwordChangeHandler = useCallback((passwordText) => {
        ...
        if(passwordText.length < 8 || passwordText.length > 20) {
            setPasswordNameDescription('비밀번호는 8자리 이상 20자리 이하입니다.');
            setIsBtnDisabled(true);
        }else {
                ...
                <Container>
                    ...
                    >
                        ...
                        {passwordDescription ? (
                            ...
                            <AlertMsg>띄어쓰기 없는 8-20자리 영어 대소문자와 숫자 조합으로 입력해주세요.</AlertMsg>
                        )}
