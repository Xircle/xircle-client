...
import airpod_event_1 from '../../../static/images/my-profile/airpod_event_1.svg'
import airpod_event_2 from '../../../static/images/my-profile/airpod_event_2.svg'
import airpod_event_3 from '../../../static/images/my-profile/airpod_event_3.svg'
...
const heightGenerator = (idx: number) => {
...
const MyProfile = () => {
    ...
    const [naviContents, setNaviContents] = useState<Element | null>(null);
    const [articleClickedArr, setArticleClicked] = useState([false, null]);
    const statusProfile = useAppSelector(store => store.profile.status);

    const articles = useAppSelector(store => store.articles.articles);
    const statusArticle = useAppSelector(store => store.articles.status);
    ...
    //             // dispatch(actions.getInterestArticle("술/맛집탐방", token));
    //         }

    //         let foundArr = [];
    //         if(articleArrInProfile["술/맛집탐방"]) { // fetch된 후
    //             foundArr = articleArrInProfile["술/맛집탐방"]; // [ {interest: '스타트업', articleImgSrc: 'www.api.xircle~', articleTitle: "안녕하세요~"} ]
    //         }
    //         setNaviContents(
    //             articleIsLoading ? (
    //                 <div style={{height: 100, position: 'relative'}}>
                    ...
                    {/* 프로필 사진 */}
                        ...
                        <div className="mt-3">
                            <div className="text-center">
                                ...
                                <h3 style={{fontSize: 20, fontWeight: '600', margin: '5px 0'}}>{adj} {job} <span style={{fontSize: 18, fontWeight: 'lighter'}}>{displayName}</span></h3>
                            </div>
                            <div className={`flex flex-row justify-center`}>
                                {isPublic ? <div style={{height: 25, borderRadius: 3, backgroundColor: isGraduate ? 'rgb(204, 246, 255)' : "#DAD4FF", margin: '0 2px'}}><p style={{fontSize: '8px', color: '#616060', fontWeight: 'normal', padding: '7px'}}>{univ} {isGraduate ? "졸업" : "재학중"}</p></div> : null}
            ...
            <header style={{margin: "20px 0 35px 0"}}>
                <section className="flex flex-row items-center justify-around mt-1">
                    ...
                    <button 
                        ...
                        onClick={() => setPageNum(2)}
                    ><p style={{fontSize: 18, fontWeight: 300}}>{displayName}</p>
                    </button>
                    <button 
                        className="px-5 rounded-3xl focus:outline-none"
